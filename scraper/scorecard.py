...
def compute_gate_throughputs(
    ...
    """
    Compute Delta C(k) = C(k, curr) - C(k, base) across all 4 gates.
    Also calculates normalized rates, composite score, and accountability lines.
    """
    c_curr = compute_cumulative_reverse_stocks(curr_metrics)
    c_base = compute_cumulative_reverse_stocks(base_metrics)
    
    # Raw Gate Crossings
    g1 = c_curr["C_G1"] - c_base["C_G1"]  # Crossed out of No Action into LCO+
    g2 = c_curr["C_G2"] - c_base["C_G2"]  # Crossed out of LCO Proposal into GA Auth+
    g3 = c_curr["C_G3"] - c_base["C_G3"]  # Crossed out of Pre-Trial into HC Hearing+
    g4 = c_curr["C_G4"] - c_base["C_G4"]  # Crossed into Closed / Completed
    
    # Stage-Specific Opening balances for rate normalization
    curr_pending = int(curr_metrics.get("total_cases_pending") or 0)
    base_pending = int(base_metrics.get("total_cases_pending") or curr_pending or 0)
    
    base_no_act = int(base_metrics.get("no_action_taken") or 0)
    base_lco = int(base_metrics.get("lco_proposal_stage_pending") or 0)
    base_pre_trial = (
        int(base_metrics.get("draft_pwr_pending") or 0) +
        int(base_metrics.get("approved_pwr_pending") or 0) +
        int(base_metrics.get("affidavit_filing_pending") or 0)
    )
    base_final_orders = int(base_metrics.get("final_order_compliance_pending") or 0)
    
    # Normalized conversion rates against Stage-Specific Opening Balance (capped at 1.0 = 100%)
    rate_g1 = min(1.0, max(0, g1) / max(1, base_no_act)) if (base_no_act > 0 or g1 > 0) else 0.0
    rate_g2 = min(1.0, max(0, g2) / max(1, base_lco)) if (base_lco > 0 or g2 > 0) else 0.0
    rate_g3 = min(1.0, max(0, g3) / max(1, base_pre_trial)) if (base_pre_trial > 0 or g3 > 0) else 0.0
    rate_g4 = min(1.0, max(0, g4) / max(1, base_final_orders)) if (base_final_orders > 0 or g4 > 0) else 0.0
    
    # Composite Score (Weights: 0.10 G1, 0.15 G2, 0.45 G3, 0.30 G4)
    composite_score = (
        (0.10 * rate_g1) +
        (0.15 * rate_g2) +
        (0.45 * rate_g3) +
    ...
    # Total volume of positive gate crossings
    total_crossings = max(0, g1) + max(0, g2) + max(0, g3) + max(0, g4)
    
    # Accountability lines
    net_backlog_delta = curr_pending - base_pending
    net_backlog_minus_intake = net_backlog_delta - intake_inflow
    
    curr_no_act = int(curr_metrics.get("no_action_taken") or 0)
    ...
    return {
        ...
        "base_lco": base_lco,
        "base_pre_trial": base_pre_trial,
        "base_final_orders": base_final_orders,
        ...
        "net_backlog_minus_intake": net_backlog_minus_intake,
